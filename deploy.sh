...
# ===================== 安装Realm（国外VPS最终版：适配新文件名+移除误判预检测+保留核心下载）=====================
install_realm() {
    info "🔍 检测Realm是否安装..."
    ...
    log "Realm未安装，执行GitHub官方二进制包安装（适配新文件名x86_64/aarch64，国外VPS专属）"
    
    # 检测系统架构 + 精准匹配官方新文件名（核心）
    ARCH=$(uname -m)
    if [ "$ARCH" = "x86_64" ]; then
    ...
    log "系统架构：${ARCH} → 官方新包标识：${REALM_ARCH_FULL}"

    # 官方最新有效下载链接（适配新文件名，手动测试过可访问）
    ...
    # 核心wget下载：保留所有验证过的有效参数（手动测试可行，直接复用）
    info "🔗 从GitHub下载Realm官方新包（处理重定向+显示进度+自动重试）..."
    if wget --no-check-certificate -L -O ${REALM_TMP} ${GITHUB_URL} --show-progress --timeout=20 --tries=5; then
        green "✅ Realm新包下载成功（文件路径：${REALM_TMP}）"
    else
        red "❌ Realm新包下载失败！（但手动测试可行，可忽略此错误直接执行手动安装）"
        red "   紧急手动安装命令（直接复制执行，适配当前架构）："
        red "   wget --no-check-certificate -L -O /tmp/realm.tar.gz ${GITHUB_URL} && tar -zxf /tmp/realm.tar.gz -C /tmp && mv /tmp/realm /usr/local/bin/ && chmod +x /usr/local/bin/realm && rm -f /tmp/realm.tar.gz"
        log "错误：wget下载${GITHUB_URL}失败，建议执行上述手动命令"
        rm -f ${REALM_TMP}
        exit 1
    fi

    # 校验下载包完整性（避免损坏包，正常≥10M）
    info "✅ 校验下载包完整性..."
    if [ ! -f ${REALM_TMP} ] || [ $(du -k ${REALM_TMP} | awk '{print $1}') -lt 10240 ]; then
        red "❌ 下载包损坏/不完整（正常大小≥10M，当前大小异常）"
        log "错误：Realm包大小异常，可能是下载中断导致"
        rm -f ${REALM_TMP}
        exit 1
    fi
    ...
    if command -v realm &>/dev/null; then
        ...
        log "Realm安装成功，架构标识：${REALM_ARCH_FULL}"
    else
