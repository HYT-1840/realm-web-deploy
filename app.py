...
# 初始化Flask应用
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('REALM_SECRET_KEY', 'realm-web-2026-custom-key')  # 建议部署时通过环境变量设置
...
def start_realm(local_port, target):
    ...
    # 验证realm命令是否存在
    if subprocess.call(['which', realm_cmd], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) != 0 and sys.platform != 'win32':
        return False, "未找到realm命令，请确认已安装"
    ...
    try:
        ...
        if psutil.pid_exists(p.pid):
            ...
            return False, "进程启动后立即退出"
    except FileNotFoundError:
        return False, "未找到realm可执行文件，请确认已安装"
    except Exception as e:
...
def api_add_user():
    ...
    if len(username) < 3 or len(password) < 6:
        return jsonify({'code': 1, 'msg': '用户名至少3位，密码至少6位'})
...
def api_add_rule():
    ...
    try:
        ...
        return jsonify({'code': 1, 'msg': '端口必须为数字'})
    ...
    if db_query('SELECT * FROM realm_rules WHERE local_port=?', (local_port,)):
        return jsonify({'code': 1, 'msg': '端口已配置规则'})
    if is_port_used(local_port):
        return jsonify({'code': 1, 'msg': '端口已被其他进程占用'})
...
def api_start_rule():
    ...
    try:
        ...
        return jsonify({'code': 1, 'msg': '规则ID必须为数字'})
    ...
    # 再次检测端口（防止规则添加后端口被占用）
    if is_port_used(rule['local_port']):
        return jsonify({'code': 1, 'msg': '端口已被占用，启动失败'})
...
def api_stop_rule():
    ...
    stop_result = stop_process(rule['pid'])
